...
/*  29 */   public HashSet<Chunk> lobbychunks = new HashSet<Chunk>();
 
/* 164 */   boolean showingMessage = false;
/* 165 */   ArrayList<String[]> messagequeue = new ArrayList<String[]>(3);
...
/*  64 */     byte temp = ((Sign)new Location(this.p.getServer().getWorld(c.getString("sg-system.lobby.sign.world")), x1, y1, z1).getBlock().getState()).getData().getData();
     Location l;
/*  66 */     if ((temp == 3) || (temp == 4)) {
/*  67 */       l = new Location(Bukkit.getWorld(c.getString("sg-system.lobby.sign.world")), x1, y1, z1);
/*  68 */       inc = -1;
     } else {
...
/* 273 */       this.tid = Bukkit.getScheduler().scheduleSyncDelayedTask(this.p, new Runnable() {
         public void run() {
/* 275 */           Bukkit.getScheduler().scheduleSyncRepeatingTask(LobbyManager.this.p, new LobbyManager.LobbySignUpdater(), 1L, 20L);
...
/* 354 */         this.signs[b][1].setLine(1, game.getMode().toString());
