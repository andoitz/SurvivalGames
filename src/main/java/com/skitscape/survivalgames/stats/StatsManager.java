...
/*  31 */   private ArrayList<PreparedStatement> queue = new ArrayList<PreparedStatement>();
 
/*  33 */   private DatabaseDumper dumper = new DatabaseDumper();
 
/*  35 */   private DatabaseManager dbman = DatabaseManager.getInstance();
 
/*  39 */   private HashMap<Integer, HashMap<Player, PlayerStatsSession>> arenas = new HashMap<Integer, HashMap<Player, PlayerStatsSession>>();
...
/*  79 */           s1.execute();
   ...
   {
/*  90 */     this.arenas.put(Integer.valueOf(arenaid), new HashMap<Player, PlayerStatsSession>());
   }
 
   public void addPlayer(Player p, int arenaid)
   {
/*  96 */     ((HashMap<Player, PlayerStatsSession>)this.arenas.get(Integer.valueOf(arenaid))).put(p, new PlayerStatsSession(p, arenaid));
   }
...
/* 160 */     for (PlayerStatsSession s : this.arenas.get(Integer.valueOf(arenaid)).values()) {
...
/* 192 */     public void run() { while (StatsManager.this.queue.size() > 0) {
